...
- `pyproject.toml` — declara as dependências do projeto:
  ...
  - `pandas` — usado pelo `identificar_extensoes.py`.
...
- `download.py` — lê `fontes/estimativas-de-populacao.csv` (colunas `caminho,nome,tipo,url` geradas pelo `gerar_fontes.py`) e baixa todos os arquivos, replicando a árvore em `downloads/`:
...
- `gerar_fontes.py` — lê um arquivo `.html.md` (por padrão, `fontes/estimativas-de-populacao.html.md`, ou o caminho passado como argumento posicional):
